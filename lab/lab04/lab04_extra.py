...
def merge(lst1, lst2):
    ...
    lst12 = lst1 + lst2
    while lst12:
        results = results + [min(lst12)]
        lst12 = [i for i in lst12 if i != min(lst12)]
    return results
...
def put_piece(board, max_rows, column, player):
    ...
    if max_rows:
        if board[rows][column] == '-':
            return rows, replace_elem(board,rows, replace_elem(board[rows],column,player))
        else:
            return put_piece(board, rows, column, player)
    else:
        return -1, board
...
def check_win_row(board, max_rows, max_cols, num_connect, row, player):
    ...
    for cols in range(max_cols):
        if get_piece(board, row, cols) == player:
            count = count + 1
...
def check_win_column(board, max_rows, max_cols, num_connect, col, player):
    ...
    for rows in range(max_rows):
        if get_piece(board, rows, col) == player:
            count = count + 1
            if count == num_connect:
                return True
        else:
