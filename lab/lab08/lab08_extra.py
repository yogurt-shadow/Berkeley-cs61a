...
def make_to_string(front, mid, back, empty_repr):
    ...
    """
    "*** YOUR CODE HERE ***"
    def string_show(link):
        i = 0
        if link is Link.empty:
            return empty_repr
        else:
            i += 1
            link_str = front + str(link.first) + mid + string_show(link.rest)
            return link_str + back * i
    return string_show
...
def tree_map(fn, t):
    ...
    if t.is_leaf():
        ...
        branch = [tree_map(fn, branch) for branch in t.branches]
        return Tree(new_label, branch)
...
def long_paths(tree, n):
    ...
    if tree.is_leaf():
        if n <= 0:
            result.append(Link(tree.label))
    for branch in tree.branches:
        for ele in long_paths(branch, n-1):
            result.append(Link(tree.label, ele) )
    return result
